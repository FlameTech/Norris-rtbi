...
/**
 * Description: this function prepares the template for the line chart's options
 * @method buildTempl
 * @return JSON
 */
...
/**
 * Description: this function fills lineOptions with default options
 * @method fillDefaultOpts
 * @param { Object } lineOptions
 * @param { Number } seriesNum
 * @return void
 */
var fillDefaultOpts = function(lineOptions, seriesNum) {
  var series = [];
  ...
  lineOptions.colors = colorManager.colorGenerator(seriesNum);
...
/**
 * Description: this function fills lineOptions with the dev's options
 * @method fillDevOpts
 * @param { JSON } lineOptions
 * @param { JSON } options
 * @param { Number } seriesNum
 * @return void
 */
var fillDevOpts = function(lineOptions, options, seriesNum) {
  if(options.series) {
...
/**
 * Description: this function fills lineOptions with the dev's color options
 ...
 * @return void
 */
var fillDevColorOpts = function(lineOptions, options, seriesNum) {
...
/**
 * Description: this method creates a line chart with the specified parameters
...
exports.createLineChart = function(title, xAxisName, yAxisName, labels, data, options) {
  ...
  if(title==null || typeof title !== "string") {new NorrisError(4000);}
    ...
    var lineOptions = buildTempl();
    //check optional parameters 
    if(options===undefined) {
      fillDefaultOpts(lineOptions, data.length);
    }
    else if(!(dataConsistency.jsonConsistencyCheck(options, lineOptions))) {
      new NorrisError(1001);
    }
    else {
      fillDevOpts(lineOptions, options, data.length);
      fillDevColorOpts(lineOptions, options, data.length);
    }
...
exports.updateInPlace = function(graphID, label, set, newValue) {
  ...
  if(set < _lineChartModel._data.length && set >= 0) {
    ...
    console.log("Warning: invalid data set for LineChart inPlace update. \n Update rejected.");
  }
};

/**
 * Description: this function allows to insert a new value into data
 * @method updateStream
 * @param { Number } graphID
